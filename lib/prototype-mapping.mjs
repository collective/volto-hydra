...
function topNodes(markdown) {
  return mdParser.parse(markdown).children.map((n) => {
    // A paragraph that is only an image or only a link is that node's own block
    // (a standalone image, or an object-browser link item), so lift it out of
    // the wrapping paragraph. (No slate block in the content is a lone link, so
    // this does not steal one from `${p/slate}`.)
    if (n.type === 'paragraph' && n.children?.length === 1
        && (n.children[0].type === 'image' || n.children[0].type === 'link')) {
      return n.children[0];
    }
    return n;
  });
...
function resolvePart(node, ref) {
  switch (ref.part) {
    ...
    // A link ITEM: the object-browser value with the picked item's title, which a
    // teaser renders (`href[0].title`) when `overwrite` is off -- so the link
    // text carries it. `@id`-only `/link` (a button) drops it, since a button
    // renders only the url.
    case 'linkitem': {
      const u = findUrl(node);
      if (u === null) throw new Error(`no link url in ${kindOf(node)}`);
      return [{ '@id': u, title: nodeText(node) }];
    }
...
// If `proto`'s one ref-field is the star-slate ref (`${` star `/slate}`) -- the
// whole light-tag body mapped into a single (possibly multi-node) slate value --
// return its field name, else null.
function wholeSlateField(proto) {
  const refs = Object.entries(proto.fields).filter(([, v]) => parseRef(v));
  if (refs.length !== 1) return null;
  const [field, v] = refs[0];
  const ref = parseRef(v);
  return ref.node === '*' && ref.part === 'slate' ? field : null;
}
...
function decodeTag(protos, attrs, inner, depth) {
  ...
  for (const proto of leaves) {
    ...
    if (run && run.length === content.length) return finish(leafBlock(proto, content, depth));
  }
  // A whole-body slate: `value="${*/slate}"` maps EVERY body node into one
  // (multi-node) value -- the "badly formed" slate that can't be bare markdown
  // because it would split into a block per top-level node.
  const whole = protos.filter((p) => p.type === type).map(wholeSlateField).find(Boolean);
  if (whole && content.length) {
    return finish({ '@type': type, [whole]: content.flatMap((n) => blockToSlate(n)) });
  }
...
function emitNode(block, entry, depth) {
  ...
  if (p.link) {
    ...
    if (entry.node === 'img') return `![${p.alt ? block[p.alt] : ''}](${url})`;
    const link = `[${p.text ? block[p.text] ?? '' : ''}](${url})`;
    return /^h/.test(entry.node) ? `${hashes(entry.node, depth)} ${link}` : link;
  }
  // A link item renders its own title as the link text: `[title](@id)`.
  if (p.linkitem) {
    const v = block[p.linkitem][0];
    return `[${v.title ?? ''}](${v['@id']})`;
  }
...
// A link value carries an `@id` and NOTHING BUT summary keys -- so both the
// clean form ({@id, title}) and the stored brain ({@id, Title, ...}) reduce to
// the same {@id}, symmetrically. A content item (a tab, a panel) has content
// keys (blocks, value, label, code) outside this set and is never collapsed.
