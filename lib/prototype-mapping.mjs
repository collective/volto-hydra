...
function decodeTag(protos, attrs, inner, depth) {
  ...
  if (container) {
    ...
    for (const region of container.regions) setPath(block, region.name, decodeRegion(protos, region, inner, depth));
  ...
  for (const proto of leaves) {
    const run = runAt(inner, 0, slotsOf(proto), depth);
    if (run && run.length === inner.length) {
      const block = leafBlock(proto, inner, depth);
...
function emitFields(proto, obj, depth) {
  ...
  return nodes.map((e) => emitNode(obj, e, depth)).join('\n\n');
}
...
export function emitBlocks(prototypes, blocks, depth = 2) {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    const gens = isContainer
      ...
      : [() => emitBlock(prototypes, block, depth), () => emitTag(prototypes, block, depth), () => emitRawTag(block)];
    let chosen = null;
