...
export function parsePrototypes(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (tag.name === 'block') {
      // `explicit` is a structural marker, not a field: an explicit prototype
      // matches ONLY inside its `<block type>` tag, never bare markdown -- so a
      // pattern that overlaps a common one (a heading-link is also a teaser) does
      // not silently grab it. Bare stays the implicit/common reading.
      const { type, explicit, ...fields } = attrs;
      const proto = { type, explicit: explicit === true, fields, regions: [] };
...
function matchRun(protos, nodes, i, depth) {
  const ordered = protos.filter((p) => isLeaf(p) && !p.explicit)
