...
import { tagOf, mdParser, blockToSlate, slateToMd, fmtTagAttrs } from './blockmd.mjs';
...
const nodeInSlot = (node, kinds) => kinds.includes('*') || kinds.includes(kindOf(node));
...
function resolveRef(run, ref) {
  ...
  const node = run.filter((n) => nodeInSlot(n, kinds))[(ref.index ?? 1) - 1];
...
function decodeItem(protos, item, group) {
  ...
  for (const [field, val] of Object.entries(item.fields)) {
    ...
    const node = group.filter((n) => nodeInSlot(n, ref.node.split('|')))[(ref.index ?? 1) - 1];
  ...
  if (remainder.length) result.blocks = matchNodes(protos, remainder);
  return result;
}
...
function decodeTag(protos, attrs, inner) {
  ...
  for (const proto of leaves) {
    const run = runAt(inner, 0, slotsOf(proto));
  ...
  while (i < nodes.length) {
    ...
    if (n.type === 'html') {
      ...
      if (tag && tag.name === 'block' && tag.kind === 'self') { // tier-3 data tag
        blocks.push(decodeRawTag(tag.attrs));
        i += 1;
        continue;
      }
...
export function decodePage(md) {
  ...
  const keyed = keyBlocks(matchBlocks(protos, md.slice(m[0].length)), assignments ?? []);
...
function emitFields(proto, obj) {
  ...
  return nodes.map((e) => emitNode(obj, e)).join('\n\n');
}
...
function emitOverrides(proto, block) {
  ...
  for (const [k, v] of Object.entries(block)) {
    ...
    if (def !== undefined && !parseRef(def) && deepEqual(def, v)) continue; // matches default
    if (typeof v === 'string') out.push(`${k}="${v}"`);
  }
  return out.length ? ` ${out.join(' ')}` : '';
}

/** Emit one repeating item: its ref'd nodes, then its remainder blocks if any. */
function emitItem(protos, itemProto, item) {
  const fields = emitFields(itemProto, item);
  const rest = item.blocks ? emitBlocks(protos, item.blocks) : '';
...
function emitBlock(protos, block) {
  ...
  if (container) return emitContainer(protos, container, block);
  if (protos.some((p) => p.type === block['@type'] && isLeaf(p))) return emitLeaf(protos, block);
...
const roundTrips = (protos, md, expected) => {
  try { return semanticEqual(matchBlocks(protos, md), expected); } catch { return false; }
};
...
export function emitBlocks(prototypes, blocks) {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    // A container is already a tag, so light-wrapping it would double-nest.
    const candidates = isContainer
      ? [emitBlock(prototypes, block), emitRawTag(block)]
      : [emitBlock(prototypes, block), emitTag(prototypes, block), emitRawTag(block)];
    const chosen = candidates.find((c) => roundTrips(prototypes, [...parts, c].join('\n\n'), expected));
    parts.push(chosen ?? emitRawTag(block));
  }
  return parts.join('\n\n');
}

/** A keyed object_list region: items each with an `@id` and their own layout. */
const isKeyedItemArray = (v) => Array.isArray(v) && v.length > 0
  && v.every((el) => el && typeof el === 'object' && el['@id'] !== undefined && el.blocks && el.blocks_layout);

/**
 * Reverse of `keyBlocks`: read a keyed page (blocks dict + blocks_layout, with
 * keyed object_list items) back into the ordered block list `emitBlocks` takes,
 * collecting the uids/@ids into an assignments list in the same depth-first
 * order `keyBlocks` consumes. The uids are read straight off the keyed shape --
 * nothing is reconstructed. The input is not mutated.
 */
export function unkeyBlocks(keyed) {
  const assignments = [];
  const unkeyList = ({ blocks, blocks_layout }) => blocks_layout.items.map((uid) => {
    ...
    for (const [field, val] of Object.entries(block)) {
      if (!isKeyedItemArray(val)) continue;
      block[field] = val.map((item) => {
        assignments.push({ id: item['@id'] });
        const { '@id': _id, blocks_layout: _bl, blocks: childBlocks, ...rest } = item;
        return { ...rest, blocks: unkeyList({ blocks: childBlocks, blocks_layout: _bl }) };
      });
    }
    return block;
  });
 ...
 * (the uids the body can't hold), read straight from the keyed shape.
 */
export function emitPage(prototypes, keyed) {
  const { blocks, assignments } = unkeyBlocks(keyed);
