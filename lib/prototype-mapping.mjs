...
/** The nodes in the section under the heading whose text === `label` (up to the
 *  next heading of the same-or-higher level). `null` if no such heading -- so a
 *  `${p[Contents]}` ref scopes to that section instead of the whole body. */
...
function decodeTag(protos, attrs, inner, depth) {
  ...
  if (container) {
    ...
    for (const [field, val] of Object.entries(container.fields)) {
      ...
      const pool = ref.label ? (sectionUnder(inner, ref.label) ?? []) : inner;
