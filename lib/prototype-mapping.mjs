...
const nodeInSlot = (node, kinds) => kinds.includes('*') || kinds.includes(kindOf(node))
  || (kinds.includes('h*') && /^h[1-6]$/.test(kindOf(node))); // h* = any heading level
...
function decodeItemNode(protos, item, node) {
  const result = {};
  for (const [field, val] of Object.entries(item.fields)) {
    const ref = parseRef(val);
    if (!ref) { result[field] = val; continue; }
  ...
  for (const region of item.regions || []) result[region.name] = decodeRegion(protos, region, [node]);
  return result;
}

function decodeRegion(protos, region, inner) {
  if (region.widget === 'blocks_layout') { // a plain block sequence, with the
    return matchNodes([...protos, ...region.protos], inner); // region's scoped prototypes
  }
  const item = region.protos[0];
  if (inner.length === 1 && STRUCTURAL.has(kindOf(inner[0]))) { // iterate-children
    return inner[0].children.map((child) => decodeItemNode(protos, item, child));
  }
  const delim = delimiterOf(item);
  return splitAt(inner, delim).map((group) => decodeItem(protos, item, group));
}
...
function decodeTag(protos, attrs, inner) {
  ...
  if (container) {
    ...
    for (const region of container.regions) setPath(block, region.name, decodeRegion(protos, region, inner));
...
function matchNodes(protos, nodes) {
  ...
  while (i < nodes.length) {
    ...
    if (n.type === 'html') {
      ...
      blocks.push(decodeTag(protos, typedAttrs(n.value), nodes.slice(i + 1, j - 1)));
      i = j;
      continue;
    }
...
function emitContainer(protos, proto, block) {
  ...
  const body = proto.regions.map((region) => {
    ...
    if (region.widget === 'blocks_layout') return emitBlocks([...protos, ...region.protos], val);
    return val.map((item) => emitItem(protos, region.protos[0], item)).join('\n\n');
  }).join('\n\n');
...
export function emitBlocks(prototypes, blocks) {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    for (const gen of gens) {
      ...
      if (roundTrips(prototypes, [...parts, c].join('\n\n'), expected)) { chosen = c; break; }
    }
