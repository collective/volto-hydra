  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (open && open[1] === '::::' && open[2]) {
      if (stack.length) {
        ...
        // Two kinds of repeat. If a field takes the REST of the group as a
        // region, the group's children are `:::` directives and the scanner
        // handles them as it always has -- a heading just opens the item that
        // a `:::panel{…}` used to. Otherwise the group is scalars (a heading
        // and a fence), collected as markdown and split at the end.
        top.itemRegion = at && Object.entries(attrs).find(
          ([, v]) => v instanceof Template && v.whole?.rest);
        top.regionMd = at && !top.itemRegion ? [] : null;
        top.itemCount = 0;
      }
      continue;
    }
    ...
    if (line === ':::' || line === '::::') {
      ...
      const top = stack[stack.length - 1];
      if (line === '::::' && top?.region) {
        if (top.repeatAt && top.regionMd) {
          const md = top.regionMd.join('\n');
          const ids = top.itemTemplate['@ids'];
          top.block[top.region] = sectionsOf(md, 0, top.repeatAt).map((constructs, i) => {
            const item = {};
            if (Array.isArray(ids) && ids[i] != null) item['@id'] = ids[i];
            for (const [field, tpl] of Object.entries(top.itemTemplate)) {
              if (field === '@ids') continue;
    ...
    if (openItem && openItem.itemRegion) {
      const h = new RegExp(`^#{${openItem.repeatAt}}\\s+(.*)$`).exec(line);
      if (h) {
        if (itemOnTop) closeItem(stack.pop());
        const [regionField, regionTpl] = openItem.itemRegion;
        const item = {};
        const ids = openItem.itemTemplate['@ids'];
        if (Array.isArray(ids) && ids[openItem.itemCount] != null) {
          item['@id'] = ids[openItem.itemCount];
        }
        for (const [field, tpl] of Object.entries(openItem.itemTemplate)) {
          if (field === '@ids' || field === regionField) continue;
          item[field] = resolveOne(tpl, [{ text: h[1], level: openItem.repeatAt }]);
        }
        openItem.itemCount += 1;
        (openItem.block[openItem.region] ||= []).push(item);
        // The item behaves exactly like a block frame: `:::` children land in
        // its blocks dict and its regions become its blocks_layout.
        stack.push({ uid: '', block: item, regions: {}, region: null, isItem: true,
                     regionField: regionTpl.whole.part,
                     // Children of this item wrote their headings below the
                     // split level; lift them back by the same amount.
    ...
    if (stack.length) {
      const top = stack[stack.length - 1];
      // Inside a repeating container the body belongs to the container, not
      // to the block that holds it.
      if (top.regionMd) top.regionMd.push(raw);
