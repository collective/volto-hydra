...
function resolveRefs(block, md) {
  ...
  for (const [k, v] of Object.entries(block)) {
    ...
    if (block[head] !== undefined && typeof block[head] === 'object') {
      ...
      target[rest[0]] = resolveOne(v, constructs);
    } else {
...
function tableBody(role, b) {
  ...
  settings.keys = t.rows.map((r) => [r.key, ...r.cells.map((c) => c.key)]);
...
function sectionsBody(sec, b, schema, depth) {
  ...
  for (const item of items) {
    ...
    if (Object.keys(extra).length) {
      ...
      lines.push('', `<field ${fmtTagAttrs({ ...flat, ...deep })} />`);
    }
    if (regionField) {
...
function tagFields(attrs) {
  ...
  for (const [k, v] of Object.entries(attrs)) {
    if (k === 'data') { Object.assign(out, JSON.parse(v)); continue; }
