...
function sectionsBody(sec, b) {
  ...
  return {
    region: `::::${sec.field}[items]{${fmtAttrs({ repeat: `h${level}`, ...templateAttrs(tmpl) })}}`,
    md,
    fields: [sec.field],
  };
}
  ...
  if (b.blocks && typeof b.blocks === 'object') {
    ...
    if (!regions.length) {
      ...
      for (const region of regions) {
        ...
        if (named) lines.push(`::::${region}[blocks]`);
    ...
    if (open && open[1] === '::::' && open[2]) {
      if (stack.length) {
        ...
        if (kind !== 'blocks' && kind !== 'items') {
          throw new Error(`::::${open[2]}${open[3] ?? ''}: a region must say what it holds `
            + '-- [blocks] for the shared blocks dict, [items] for a field array.');
        }
        top.regionIsField = kind === 'items';
