 ...
 */
export function resolveMarkdownLink(url, base) {
...
export function resolveMarkdownLinksInBlocks(node, base) {
  if (Array.isArray(node)) { node.forEach((n) => resolveMarkdownLinksInBlocks(n, base)); return node; }
  if (!node || typeof node !== 'object') return node;
  if (node.type === 'link' && node.data && typeof node.data.url === 'string') {
    node.data.url = resolveMarkdownLink(node.data.url, base);
  }
  for (const value of Object.values(node)) resolveMarkdownLinksInBlocks(value, base);
  return node;
}
...
export function readTree(root, { decode = decodeAuto } = {}) {
  ...
  for (const file of walk(root)) {
    ...
    if (page.blocks) resolveMarkdownLinksInBlocks(page.blocks, relative(root, dirname(file)));
