...
/**
 * Decode one page, routing by format so a tree can hold either: a prototype page
 * declares `prototypes:`/`assignments:` in its frontmatter (the mapping the
 * engine reads); a flat page does not. Both return the same content object, so
 * the tree/server-state logic downstream is oblivious to which was used.
 */
export function decodeAuto(text) {
  const fm = text.split(/^---$/m)[1] ?? '';
  return /^\s*(blocks-matched|blocks-tagged|blocks-assignments|prototypes|assignments):/m.test(fm)
    ? decodePage(text) : mdToPage(text);
}
 ...
 * injectable so the mount and the CI agreement check share ONE tree reader while
 * the block-body format is swapped underneath -- `mdToPage` (the flat format) or
 * `decodePage` (the prototype format). The tree/server-state/blob logic below is
 * identical either way, because it works on the returned object, not the body.
 */
const LANG_BY_EXT = {
...
export function readTree(root, { decode = decodeAuto } = {}) {
  const items = new Map();
  ...
  for (const file of walk(root)) {
    ...
    // and an export — resolves them just like a `blobs:` child. Fail loud if the
    // field points at a file that is not on disk, matching the `blobs:` check.
    for (const field of Object.values(BLOB_FIELD)) {
      const bpath = page[field] && page[field].blob_path;
      if (!bpath) continue;
      const blob = join(root, bpath);
      if (!existsSync(blob)) {
        throw new Error(`${id}: ${field}.blob_path ${bpath} is not on disk`);
      }
