...
/**
 * Shadow of @plone/volto-slate/utils/blocks.js
 *
 * Fix: toggleInlineFormat now uses rangeRef when wrapping to track selection
 * through Slate normalization (which adds empty text nodes around inline elements).
 */
...
export const toggleInlineFormat = (editor, format) => {
  ...
  if (alreadyOneIsActive) {
    // Track selection through wrap/unwrap transforms - normalization adds
    // empty text nodes around inline elements, which shifts paths
    const rangeRef = Editor.rangeRef(editor, editor.selection);
    ...
    // Restore selection from tracked range
    if (rangeRef.current) {
      const newSel = JSON.parse(JSON.stringify(rangeRef.current));
      rangeRef.unref();
      Transforms.select(editor, newSel);
  ...
  // normalized
  // Track selection through wrapNodes - normalization adds empty text nodes
  // around inline elements (like <strong>), which shifts paths
  const rangeRef = Editor.rangeRef(editor, editor.selection);
