...
/**
 * Hand the browser an honest "nothing loaded yet" when the admin is
 * bridge-backed.
 *
 * asyncConnect skips its client-side load whenever reduxAsyncConnect.loaded is
 * true, trusting that the server already fetched the data. In bridge mode the
 * server fetched nothing and could not have: the adapter lives in an iframe
 * that does not exist until the browser renders one. Serialising loaded:true
 * would tell the client the data is present when the store is empty, and the
 * view would sit there waiting for content nobody ever requested.
 *
 * Only the flag is reset. Everything the server legitimately established —
 * language, site data, the user's session — is preserved.
 */
function clientInitialState(state) {
  if (!config.settings.useBridgeBackend) return state;
  return {
    ...state,
    reduxAsyncConnect: { ...state.reduxAsyncConnect, loaded: false },
  };
}
          ...
          <script
            dangerouslySetInnerHTML={{
              __html: `window.__data=${serialize(
                loadReducers(clientInitialState(store.getState())),
              )};`,
            }}
            charSet="UTF-8"
          />
