/**
 * Save-as-draft — Volto Hydra shadow.
 *
 * Same feature as core's: the form is autosaved to localStorage (debounced,
 * stamped with an autoSaveDate) and offered back if the author returns to a
 * page they left mid-edit. No API and no server storage.
 *
 * The single change is which fields are drafted — see mapSchemaToData below.
 */
...
// VOLTO-HYDRA SHADOW: draft the BLOCKS as well as the page's schema fields.
//
// Core maps the draft to `schema.properties` — the page's own fields, title and
// metadata. In Volto that is nearly everything an author types, because blocks
// are edited through a form whose fields are in the schema. In hydra almost all
// of the work happens on the CANVAS and lives in `blocks` / `blocks_layout`,
// which are not schema properties: drafting only the schema fields would offer
// an author their title back and silently drop every block they had written,
// which is worse than offering nothing.
const BLOCKS_FIELDS = ['blocks', 'blocks_layout'];
...
const mapSchemaToData = (schema, data) => {
  ...
  const keys = [
    ...
    ...BLOCKS_FIELDS.filter((k) => !Object.keys(schema.properties).includes(k)),
  ];
