// "scripts": {
  //   "main": "node ${path.join(__dirname, 'fixtures/mock-api-server.js')}",
  // },
...
function parseContentMounts() {
  ...
  if (!mountsEnv) {
    return [
      { mountPath: '/', dirPath: path.join(import.meta.dirname, 'content') },
    ];
  }
...
app.use((req, res, next) => {
  ...
  const vhmPattern =
    /^\/VirtualHostBase\/[^/]+\/[^/]+\/\+\+api\+\+\/VirtualHostRoot(.*)$/;
...
if (process.env.DEBUG) {
  app.use((req, res, next) => {
    ...
    console.log(
      `${req.method} ${req.path}${authHeader ? ' [AUTH: ' + authHeader.substring(0, 30) + '...]' : ' [NO AUTH]'}`,
    );
    next();
  });
}
...
function formatSearchItem(content, baseUrl) {
  ...
  const hasPreviewImage = !!(
    content.preview_image || content['@type'] === 'Image'
  );
  ...
  const item = {
    ...
    UID: content.UID,
    is_folderish:
      content.is_folderish !== undefined ? content.is_folderish : true,
...
function formatNavItem(rawContent, urlPath, baseUrl) {
  const hasPreviewImage = !!(
    rawContent.preview_image || rawContent['@type'] === 'Image'
  );
  return {
    '@id': `${baseUrl}${urlPath}`,
    ...
    is_folderish:
      rawContent.is_folderish !== undefined ? rawContent.is_folderish : true,
    hasPreviewImage: hasPreviewImage,
    items: [],
  };
}
...
function getNavigationItems(basePath = '/', depth = 1) {
  ...
  const baseDepth =
    normalizedBase === '/'
      ? 0
      : normalizedBase.split('/').filter((p) => p).length;
  ...
  return Object.keys(contentDirMap)
    .filter((itemPath) => {
      ...
      if (
        normalizedBase !== '/' &&
        !itemPath.startsWith(normalizedBase + '/')
      ) {
        return false;
      }
...
function generateComponents(urlPath, baseUrl) {
  ...
  return {
    ...
    navigation: {
      ...
      items: (() => {
        const hasChildren = Object.keys(contentDirMap).some(
          (p) => p !== cleanPath && p.startsWith(cleanPath + '/'),
        );
        if (hasChildren) {
...
function enrichContent(content, urlPath, baseUrl) {
  ...
  return {
    ...
    is_folderish:
      content.is_folderish !== undefined ? content.is_folderish : true,
    allow_discussion:
      content.allow_discussion !== undefined ? content.allow_discussion : false,
...
function scanContentDir(contentDirPath, mountPath) {
  ...
  console.log(
    `Scanning content from ${contentDirPath} mounted at ${mountPath}`,
  );
  ...
  dirs.forEach((dir) => {
    ...
    const isDir =
      dir.isDirectory() ||
      (dir.isSymbolicLink() && fs.statSync(fullDirPath).isDirectory());
    if (isDir) {
      ...
      if (fs.existsSync(dataPath)) {
        ...
        let contentPath = content['@id']?.startsWith('/')
          ? content['@id']
          : '/' + (content.id || dir.name);

        // Apply mount prefix
        const urlPath =
          mountPath === '/' ? contentPath : mountPath + contentPath;
...
app.post('/@login', (req, res) => {
  ...
  if (process.env.DEBUG) {
    console.log(
      `Login attempt - username: ${login}, password: ${password ? '***' : 'missing'}`,
    );
  }
  ...
  if (login && password) {
    ...
    if (process.env.DEBUG) {
      console.log(
        `Login successful, returning token: ${token.substring(0, 20)}...`,
      );
...
app.post('/*', (req, res, next) => {
  ...
  if (process.env.DEBUG) {
    console.log(
      `POST content creation: path=${req.path}, @type=${req.body?.['@type']}`,
    );
  }
  ...
  if (contentType === 'Image') {
    ...
    const imagePath =
      `${parentPath === '/' ? '' : parentPath}/${imageId}`.replace(/\/+/g, '/');
    ...
    if (process.env.DEBUG) {
      console.log(
        `Created Image: ${imagePath}${sessionId ? ` (session: ${sessionId})` : ''}`,
      );
    }
  ...
  return res
    .status(501)
    .json({ error: `Content type '${contentType}' not supported` });
});
...
app.get(/.*\/@actions$/, (req, res) => {
  // Handle ++api++ prefix
  const cleanPath =
    req.path.replace('/++api++', '').replace('/@actions', '') || '/';
    ...
    if (searchPath === '' || searchPath === '/') {
      ...
      if (searchContent && searchContent.is_folderish) {
        ...
        items = Object.keys(contentDirMap)
          ...
          .map((itemPath) =>
            formatSearchItem(loadContentFromDisk(itemPath), baseUrl),
          );
      } else {
...
app.get('*', (req, res, next) => {
  ...
  if (content) {
    ...
    if (process.env.DEBUG) {
      console.log(
        `[DEBUG] Serving API content for ${cleanPath} (auth: ${authenticated})${sessionId ? ` (session: ${sessionId})` : ''}`,
      );
      console.log(`[DEBUG] Query params:`, req.query);
      console.log(
        `[DEBUG] Response preview:`,
        JSON.stringify(filteredContent).substring(0, 500),
      );
    }
...
const FRONTEND_DIR = path.join(import.meta.dirname, 'test-frontend');
app.use(
