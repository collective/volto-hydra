...
<!-- file: examples/astro/HighlightBlock.astro -->
```astro
---
/**
 * Highlight block. Background is either an image or a gradient chosen
 * from a small palette by the editor's color picker. Description is a
 * slate value, and there's an optional CTA link.
 */
...
const gradient = gradients[block.styles?.descriptionColor] || 'linear-gradient(135deg, #334, #556)';
const bgStyle = block.image
  ? `background-image:url(${block.image});background-size:cover;background-position:center`
  : `background:${gradient}`;
const description = block.description || [];
---
