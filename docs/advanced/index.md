...
As soon as the editor logs into the Hydra editor, it will load up the frontend into an iframe. Your frontend should now use the same auth token so you access the REST API with the same privileges and can render the same content including private content.
...
Use the `getAccessToken()` helper from hydra.js:
...
### Example: Next.js 14 with ploneClient
...
## Preventing Reloads (TODO)

If you wish to make the editing experience smoother, you can register for `onRoute` callbacks to prevent the frontend being forced to reload at certain times using the Hydra editor.
